...
'''The Status box is not manually scrolled by
users; the text list for the status box is
instead rearranged by ui logic in response to
user interactions with the INIT box;
therefore we need not track a lower index for
displayed text, as it will always be 0 '''
...
BOX_HEADER_PADDING = 4  # the number of spaces from the visual top of a box to leave before we render the box content text # noqa: E501
BOX_HEIGHT_PADDING = 5  # BOX_HEADER_PADDING plus one for the border line at the visual bottom of a box # noqa: E501
BOX_BORDER_PADDING = 2  # constant to account for box borders in computations
CURSOR_START_PADDING = 2    # constant to indicate how many spaces from the left edge of a box to put the cursor or start text display # noqa: E501

BASE_DIR = os.path.expanduser('dm-tools/live_game/')    # top level dir of where to find files relevant to this module # noqa: E501
PCS_FILENAME = 'pcs.yaml'   # the default filename for saving/loading combatants # noqa: E501
...
CUR_BOX = None              # tracker for which box is currently selected (note: alternate box selection is currently deprecated) # noqa: E501
CUR_BOX_OPTIONS = None      # which keystroke options are available for the currently selected box # noqa: E501
...
INIT_OPTION_TUPLES = {
    ...
    '1-99': (lambda: None, 'Set Initiative'),
...
HELP_TEXT = {
    ...
    'Hp': 'Enter a number to set; prepend -/+ to adjust; integer for temp hp, float for max hp',  # noqa: E501
...
def add_character() -> str:
    ...
    if len(Game_State.initiative_list) <= Init_Cursor_Max_Upper:
        ...
        adjust_init_text_window(Init_List_Lower_Index+1,
                                Init_List_Upper_Index+1)
...
def add_condition() -> str:
    ...
    :return: a log message string
    '''
    display_help_text('; '.join([HELP_TEXT['Conditions'],
                                 HELP_TEXT['Cancel']]))
...
def adjust_init_text_window(lower_val, upper_val) -> None:
    ...
    Init_Box_Text = Game_State.initiative_list[Init_List_Lower_Index:Init_List_Upper_Index+1] if len(Game_State.initiative_list) > 0 else []  # noqa: E501
    INIT_BOX.clear()
...
def auto_initiative() -> str:
    ...
    for pc in Game_State.pcs:
        ...
        result = roll + bonus
        Game_State.set_initiative(pc, result)
...
def execute_box_choice(text: str) -> str:
    ...
    if text is None or len(text) < 1:
        ...
        name = ' '.join(name.split()[1:])
        value = int(text)
        Game_State.set_initiative(name, value)
        INIT_BOX.clear()
...
def legend_box_text() -> List[str]:
    ...
    modified_kv = [(k, v) for (k, v) in Game.damage_types.items() if k != "bludgeoning, piercing, and slashing from nonmagical weapons that aren't silvered"]  # noqa: E501
...
def max_len_append(new_item: Any,
                   the_list: List[Any],
                   max_len: int
                   ) -> List[str]:
    '''
    ...
    :return: a list of length <= max_len
    '''
    if new_item is None:
        return the_list
...
def nav_conditions(name: str, conds: str) -> str:
    ...
    :return: a single-character string that contains the emoji corresponding
    ...
    conditions_index_for_top_entry = 3
    ...
    while key not in FINAL_KEYS:
        if key in RIGHT_LEFT_KEYS:
            if key == 'KEY_RIGHT' and index < len_string-1:
                if string[index] != ' ' and \
                   unicodedata.east_asian_width(string[index]) == 'W':
                    buf += 2
                else:
                    buf += 1
                index += 1
...
def navkey_to_index(keystroke: str,
                    text_list: List[str],
                    cursor_index: int,
                    box: Any
                    ) -> int:
    ...
    :param cursor_index: an integer indicating where in the box the cursor
                         currently sits
    :return: an integer representing the new cursor position in the displayed
              window
    '''
    ...
    if keystroke == 'KEY_UP' and cursor_index <= 0:
        if relevant_lower <= 0:
            ...
            return 0
                                      ...
                                      (len_list == relevant_max+1 and cursor_index >= relevant_upper-1) or  # noqa: E501
                                      (len_list > relevant_max+1 and cursor_index >= relevant_max and relevant_upper == len_list)):  # noqa: E501
...
def remove_character(name: Optional[str] = None) -> str:
    ...
    global Init_Box_Text
    if len(Init_Box_Text) < 1:
        return 'no combatants to remove'
...
def render_box(box: Any,
    ...
    box.addstr(CURSOR_START_PADDING, 1, ''.join('-' for i in range(width-CURSOR_START_PADDING)))  # noqa: E501
    ...
    for string in strings:
        ...
        try:
            if len(string) >= width-BOX_HEADER_PADDING:
                box.addstr(start,
                           CURSOR_START_PADDING,
                           string[:width-BOX_HEADER_PADDING])
            else:
...
def render_box_highlight_text(box: Any,
                              height: int,
                              width: int,
                              title: str,
                              strings: List[str],
...
def scroll_legend_down() -> None:
    ...
    if Legend_Text_High > len(Legend_Box_Text):
        Legend_Text_Low = 0
        Legend_Text_High = Legend_Box_Height - BOX_BORDER_PADDING - \
            BOX_HEIGHT_PADDING
...
def set_hp(text: Optional[str] = None) -> str:
    ...
    Game_State.update_hp(name, change, change_type, target, WRITE_CHANGES)
...
def set_init_bonus(text: Optional[str] = None) -> str:
    ...
    ret_msg = 'set initiative bonus'
    if len(Init_Box_Text) < 1:
        return ': '.join([ret_msg, 'no combatants present'])
    name = Init_Box_Text[Cursor_Index]
    name = ' '.join(name.split()[1:])
    ...
    if not is_integer(change):
        return ': '.join([ret_msg, 'non-integer input'])

    success = Game_State.set_init_bonus(name, int(change))

    STATUS_BOX.clear()
    return ': '.join([ret_msg, success])
...
def sort_init_list() -> str:
    ...
    '''
    global Init_Box_Text
    Game_State.sort_initiative_list()
    INIT_BOX.clear()
...
def main(pcs, write_changes):
    ...
    Menu_Box_Height = len(Menu_Box_Text) + BOX_BORDER_PADDING + \
        BOX_HEIGHT_PADDING
    MENU_BOX = curses.newwin(Menu_Box_Height-BOX_BORDER_PADDING,
                             Menu_Box_Width, 0, 0)
    ...
    Init_Cursor_Max_Upper = Height - BOX_HEADER_PADDING - INPUT_PANEL_HEIGHT \
        - HELP_PANEL_HEIGHT
    ...
    Status_Cursor_Max_Upper = Height - (BOX_HEIGHT_PADDING +
                                        INPUT_PANEL_HEIGHT)
    ...
    while True:
        ...
        if curses.is_term_resized(h, y):
            ...
            Init_Box_Width = 40 if Width > 120 else (Width // 3) - \
                BOX_BORDER_PADDING
            ...
            Init_Cursor_Max_Upper = Height - BOX_HEADER_PADDING - \
                INPUT_PANEL_HEIGHT - HELP_PANEL_HEIGHT
            Init_List_Lower_Index = 0
            Init_List_Upper_Index = min(Init_Cursor_Max_Upper,
                                        max(len(Game_State.initiative_list)-1,
                                            Init_List_Lower_Index))
        ...
        if len(Init_Box_Text) > 0:
            ...
            adjust_init_text_window(Init_List_Lower_Index,
                                    Init_List_Upper_Index)
            STATUS_BOX.clear()
...
@click.option('--pcs', '-p', type=click.Path(dir_okay=False,
              ...
              default=f'{BASE_DIR}{YAML_DIR}{PCS_FILENAME}',
